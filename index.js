...
app.get("/jokes/:id", (req, res)=>{
  ...
  var specificJoke = jokes.find((joke) => joke["id"] === id);
...
app.get("/filter", (req, res) =>{
  ...
  */
  const jokesArr = jokes.filter((joke) => joke["jokeType"] == req.query["type"]);
  res.json(jokesArr);
})
...
app.put("/jokes/:id", (req, res) =>{
  const index = jokes.findIndex((joke) => joke["id"] === parseInt(req.params["id"]));
  const putEntry = {
    id: parseInt(req.params["id"]),
    jokeText: req.body["text"],
    jokeType: req.body["type"]
  };
  jokes[index] = putEntry;
...
app.delete("/jokes/:id", (req, res)=>{
  ...
  //check if entry exist/found first
  if(index > -1){
